...
* Используется Redis для работы с очередями
...
* Асинхронная отправка сообщений по фильтрам
...
* `app/Jobs/` - Задачи для очереди (асинхронное выполнение)
  * `LogJob.php` - запись в лог
  * `NotificationJob.php` - рассылка опубликованного объявления по фильтрам
* `app/Models/` - Модели
